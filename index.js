...
class KeyboardVirtual {
  ...
  createButtons() {
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      for (let j = 0; j < rowNumbers[i]; j += 1) {
        ...
        const spanLtUp = document.createElement('span');
        const spanLtDown = document.createElement('span');
        ...
        spanLt.className = keyboardKeys[i][j][1] + langOff;

        key.append(spanEn);
        key.append(spanLt);

        spanLtDown.className = 'case-shown';
        spanLt.append(spanLtDown);
        spanLtDown.insertAdjacentText('afterbegin', keyboardKeys[i][j][2]);

        spanLtUp.className = 'case-hidden';
        spanLt.append(spanLtUp);
        spanLtUp.insertAdjacentText('afterbegin', keyboardKeys[i][j][3]);
  ...
  pageLangChanging() {
    if (localStorage.getItem('virtualKeyboardLang') === 'EN') {
      ...
      localStorage.setItem('virtualKeyboardLang', 'LT');
    } else if (localStorage.getItem('virtualKeyboardLang') === 'LT') {
      localStorage.removeItem('virtualKeyboardLang');
      localStorage.setItem('virtualKeyboardLang', 'EN');
    }
    this.pageLangBtn.innerText = localStorage.getItem('virtualKeyboardLang');
  ...
  symbolChoise(el) {
    const [, , LtLowerCase, LtUpperCase, enLowerCase, enUpperCase] = el;
    if (localStorage.getItem('virtualKeyboardLang') === 'LT' && this.isShiftPress) {
      this.symbol = LtUpperCase;
    } else if (localStorage.getItem('virtualKeyboardLang') === 'LT' && !this.isShiftPress) {
      this.symbol = LtLowerCase;
